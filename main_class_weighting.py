...
# Class bazında orantılı split etmek için
...
experiment_name = f"class_weighting_{timestamp}"  # Deney adı
...
train_files, val_files, train_labels, val_labels = train_test_split(
    ...
    stratify=all_train_labels,  # Sınıf oranlarını koru önemli yer bura
...
class_weights = {}
# Önce class_to_idx'i burada tanımla veya MedicalImageDataset'ten al
...
class MedicalImageDataset(Dataset):
    def __init__(self, imagePaths, transform=None):
        ...
        # Etiketleri burada da alabiliriz veya __getitem__'da
        # self.labels = [self.class_to_idx[parse_image_filename(p)] for p in imagePaths if parse_image_filename(p) is not None]
    ...
    def __getitem__(self, idx):
        ...
        if numeric_label is None:
            # Bu durum aslında başta filtrelendiği için olmamalı
            # ama bir hata durumunda varsayılan bir etiket veya hata döndür
            print(
                f"Hata: Geçersiz etiketli dosya Dataloader'a geldi: {img_path}")
            # Geçici çözüm: İlk sınıfı döndür veya hata fırlat
...
# Veri dönüşümleri (Değişiklik yok)
...
# --- 5. Eğitim Döngüsü (Loss fonksiyonu ağırlıklarla güncellendi) ---
if __name__ == '__main__':
    ...
    # Ağırlık tensorunu GPU'ya gönder (eğer kullanılıyorsa)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                inputs, labels = inputs.to(device), labels.to(device)
                outputs = model(inputs)
                # Validasyon loss'u da ağırlıklı hesaplanabilir veya hesapsız olabilir.
                # Genellikle hesapsız (weightsız) loss raporlamak daha standarttır
                # ama ağırlıklı loss da modelin azınlık sınıflarına odaklanmasını
                # ne kadar başardığını gösterebilir. Şimdilik weightsız bırakalım:
                loss = nn.CrossEntropyLoss()(outputs, labels)  # Ağırlıksız loss
        ...
        val_acc = correct / total  # Doğruluk hala yanıltıcı olabilir, F1 gibi metriklere bak!
        ...
        if val_loss < best_val_loss:
            ...
            save_path = 'best_model_weighted_loss_stratified.pth'
